...
class MarketDatabase:
    ...
    def upsert_table(self, name: str, df: pd.DataFrame, key_columns: Sequence[str], delete_missing: bool = False) -> None:
        table = TABLE_MAP.get(name, name.lower())
        with self.connect() as conn:
            self._upsert_df(conn, table, df, key_columns=key_columns, delete_missing=delete_missing)
            self._ensure_indexes(conn)
            conn.commit()

    def prune_history(self, keep_last_n: int, symbols: Optional[Sequence[str]] = None) -> None:
        if keep_last_n <= 0 or not self.db_path.exists():
            return
        table = TABLE_MAP["Daily_History"]
        wanted = [str(x).upper().strip() for x in (symbols or []) if str(x).strip()]
        where_sql = ""
        params: list[object] = []
        if wanted:
            placeholders = ", ".join(["?"] * len(wanted))
            where_sql = f"WHERE UPPER(symbol) IN ({placeholders})"
            params.extend(wanted)
        params.append(int(keep_last_n))
        sql = f'''
            DELETE FROM "{table}"
            WHERE rowid IN (
                SELECT rowid FROM (
    ...
    def _prepare_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        return safe_df.where(pd.notnull(safe_df), None)

    def _table_exists_conn(self, conn: sqlite3.Connection, table: str) -> bool:
        exists = conn.execute(
            "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
            (table,),
        ).fetchone()
    ...
    def _table_columns_conn(self, conn: sqlite3.Connection, table: str) -> list[str]:
        if not self._table_exists_conn(conn, table):
            return []
        rows = conn.execute(f'PRAGMA table_info("{table}")').fetchall()
        return [str(r[1]) for r in rows if len(r) > 1]

    def _upsert_df(self, conn: sqlite3.Connection, table: str, df: pd.DataFrame, key_columns: Sequence[str], delete_missing: bool = False) -> None:
        if df is None or df.empty:
            return
        safe_df = self._prepare_df(df)
        incoming_cols = [str(c) for c in safe_df.columns]
        normalized_keys = [str(c) for c in key_columns]
        if not normalized_keys or any(key not in incoming_cols for key in normalized_keys):
            self._write_df(conn, table, safe_df, if_exists="replace")
            return
        existing_cols = self._table_columns_conn(conn, table)
        if (not existing_cols) or ("__empty__" in existing_cols) or (set(existing_cols) != set(incoming_cols)):
            self._write_df(conn, table, safe_df, if_exists="replace")
            return

        safe_df = safe_df[[col for col in existing_cols if col in incoming_cols]]
        temp_table = f"__tmp_{table}_{uuid4().hex[:12]}"
        safe_df.to_sql(temp_table, conn, if_exists="replace", index=False, chunksize=1000)
        join_sql = " AND ".join([f't."{col}" = s."{col}"' for col in normalized_keys])

        if delete_missing:
            conn.execute(
                f'DELETE FROM "{table}" AS t WHERE NOT EXISTS (SELECT 1 FROM "{temp_table}" AS s WHERE {join_sql})'
            )
        conn.execute(
            f'DELETE FROM "{table}" AS t WHERE EXISTS (SELECT 1 FROM "{temp_table}" AS s WHERE {join_sql})'
        )
        quoted_cols = ", ".join([f'"{col}"' for col in safe_df.columns])
        conn.execute(f'INSERT INTO "{table}" ({quoted_cols}) SELECT {quoted_cols} FROM "{temp_table}"')
