...
def _safe_fill_series(df: pd.DataFrame, col_name: str, default):
...
class RefreshEngine:
    ...
    def _build_universe(self) -> pd.DataFrame:
        ...
        universe["nse_symbol"] = universe.get("nse_symbol", "").astype(str).str.upper().str.strip()
        universe["bse_symbol"] = universe.get("bse_symbol", "").astype(str).str.upper().str.strip()
        universe["bse_code"] = universe.get("bse_code", "").astype(str).str.strip()
    ...
    def _build_app_input_view(self, universe: pd.DataFrame, quotes: pd.DataFrame, signals: pd.DataFrame, news_scores: pd.DataFrame) -> pd.DataFrame:
        ...
        df["headline_risk_flag"] = _safe_fill_series(df, "headline_risk_flag", "LOW")
        df["news_summary"] = _safe_fill_series(df, "news_summary", "No recent cached news")
        df["top_positive_headline"] = _safe_fill_series(df, "top_positive_headline", "")
        df["top_negative_headline"] = _safe_fill_series(df, "top_negative_headline", "")
