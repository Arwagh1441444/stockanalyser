...
DB_DEFAULT = "master_market_data.db"
...
def _safe_series(df: pd.DataFrame, col_name: str, default: str = "") -> pd.Series:
    """Return a Series even when the column is missing."""
...
class RefreshEngine:
    ...
    def refresh(self, sample_limit: Optional[int] = DEFAULT_SAMPLE_LIMIT, news_limit: int = DEFAULT_NEWS_LIMIT) -> None:
        ...
        if sample_limit is not None and int(sample_limit) > 0 and int(sample_limit) < len(universe):
            active_universe = universe.head(int(sample_limit)).copy()
        ...
        config_df = self._default_config(total_universe=total_universe, active_symbols=len(active_universe), news_limit=int(news_limit or 0))
    ...
    def _default_config(self, total_universe: int, active_symbols: int, news_limit: int) -> pd.DataFrame:
        return pd.DataFrame(
            [
                {
                    "key": "source",
                ...
                {
                    "key": "universe_mode",
                    "value": f"all NSE securities + BSE reference master, total={total_universe}, active_refresh={active_symbols}",
    ...
    def _load_existing_cache(self) -> None:
        ...
        for df_name in ["_cached_quotes", "_cached_history", "_cached_signals", "_cached_news_articles", "_cached_news_scores"]:
    ...
    def _build_universe(self) -> pd.DataFrame:
        ...
        universe["symbol"] = _safe_series(universe, "symbol").astype(str).str.upper().str.strip()
        ...
        universe["company_name"] = _safe_series(universe, "company_name").replace("", np.nan).fillna(universe["symbol"]).astype(str)
    ...
    def _fetch_nse_master(self) -> pd.DataFrame:
        started = time.perf_counter()
        try:
            r = requests.get(NSE_EQUITY_LIST_URL, headers={"User-Agent": USER_AGENT}, timeout=REQUEST_TIMEOUT)
            r.raise_for_status()
            ...
            self.log("NSE_Master", "SUCCESS", len(out), (time.perf_counter() - started) * 1000, provider="NSE")
            return out
        except Exception as exc:
    ...
    def _fetch_bse_master(self) -> pd.DataFrame:
        started = time.perf_counter()
        try:
            r = requests.get(BSE_COMPANY_LIST_URL, headers={"User-Agent": USER_AGENT}, timeout=REQUEST_TIMEOUT)
            r.raise_for_status()
            text = r.text
            ...
            self.log("BSE_Master", "SUCCESS", len(out), (time.perf_counter() - started) * 1000, provider="BSE")
            return out
        except Exception as exc:
    ...
    def _fetch_market_data(self, universe: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
        ...
        signals_cols = [
            ...
            "quote_latency_ms", "history_latency_ms"
        ]
    ...
    def _merge_with_cached_history(self, universe: pd.DataFrame, fresh: pd.DataFrame) -> pd.DataFrame:
        ...
        if not cached.empty:
            missing = set(universe["symbol"]) - set(current["symbol"].unique()) if not current.empty else set(universe["symbol"])
            cached = cached[cached["symbol"].isin(missing)]
            current = pd.concat([current, cached], ignore_index=True)
        if current.empty:
            return current
        current["date"] = pd.to_datetime(current["date"], errors="coerce")
        return current.dropna(subset=["date", "close"]).sort_values(["symbol", "date"]).drop_duplicates(subset=["symbol", "date"], keep="last").reset_index(drop=True)
    ...
    def _get_best_history(self, row: dict, from_dt: datetime, to_dt: datetime) -> Tuple[pd.DataFrame, str, List[str], float]:
        ...
        if nse_symbol:
            try:
                hist_rows = self.client.get_history(nse_symbol, from_dt, to_dt)
        ...
        for ticker, source in [(row.get("yahoo_ticker_ns"), "YAHOO_NS"), (row.get("yahoo_ticker_bo"), "YAHOO_BO")]:
            ...
            if not ticker:
                continue
            try:
                hist_df = self._history_from_yfinance(symbol=symbol, market_symbol=ticker, from_dt=from_dt, to_dt=to_dt, source_label=source)
                if not hist_df.empty:
    ...
    def _build_app_input_view(self, universe: pd.DataFrame, quotes: pd.DataFrame, signals: pd.DataFrame, news_scores: pd.DataFrame) -> pd.DataFrame:
        ...
        df["quote_source"] = df.get("quote_source", "UNAVAILABLE").fillna("UNAVAILABLE")
        ...
        df["headline_risk_flag"] = df.get("headline_risk_flag", "LOW").fillna("LOW")
        df["news_summary"] = df.get("news_summary", "No recent cached news").fillna("No recent cached news")
        df["top_positive_headline"] = df.get("top_positive_headline", "").fillna("")
        df["top_negative_headline"] = df.get("top_negative_headline", "").fillna("")
        ...
        df["news_available_weight"] = np.where(df["news_confidence"].fillna(0) >= 20, 0.22, 0.0)
        ...
        df["recommendation"] = np.select(
            [
                (df["overall_score"] >= 78) & (df["technical_score"].fillna(0) >= 65) & (df["dqi_final"].fillna(0) >= 70) & (df["ltp"].notna()) & (~high_risk),
                (df["overall_score"] >= 58) & (df["ltp"].notna()),
                (df["ltp"].isna()),
            ],
            ["BUY", "HOLD", "NO DATA"],
            default="SELL",
        )
        df["actionable_flag"] = np.where(df["recommendation"].eq("BUY"), "YES", "NO")
        df.loc[high_risk & df["ltp"].notna() & df["recommendation"].eq("BUY"), ["recommendation", "actionable_flag"]] = ["HOLD", "NO"]
        ...
        keep_cols = [
            ...
            "top_positive_headline", "top_negative_headline", "news_summary", "liquidity_score", "dqi_final", "overall_score", "recommendation", "actionable_flag", "history_source_hint", "status_note",
        ]
