...
class RefreshEngine:
    ...
    def refresh(
        ...
        try:
            ...
            news_universe = self._filter_to_symbols(active_universe, refresh_symbols)
            if news_universe.empty:
                news_universe = active_universe.copy()
            news_universe = self._apply_priority_symbols(news_universe, priority_symbols)
            if priority_symbols:
                priority_df = self._filter_to_symbols(universe, priority_symbols)
                if not priority_df.empty:
                    news_universe = pd.concat([news_universe, priority_df], ignore_index=True)
                    news_universe = news_universe.drop_duplicates(subset=["symbol"], keep="first").reset_index(drop=True)
            ...
            merged_news_articles = self._merge_with_cached_news_articles(news_result.articles)
            merged_news_scores = self._merge_with_cached_news_scores(news_result.scores)

            app_view = self._build_app_input_view(universe, quotes_df, signals_df, merged_news_scores)
            ...
            incremental_history_df = self._extract_incremental_history_rows(history_df)

            self.db.upsert_table("Universe_Master", universe, key_columns=["symbol"], delete_missing=True)
            self.db.upsert_table("Latest_Quotes", quotes_df, key_columns=["symbol"])
            if not incremental_history_df.empty:
                self.db.upsert_table("Daily_History", incremental_history_df, key_columns=["symbol", "date"])
                self.db.prune_history(HISTORY_TRADING_DAYS, symbols=active_universe["symbol"].astype(str).tolist())
            self.db.upsert_table("Latest_Signals", signals_df, key_columns=["symbol"])
            self.db.upsert_table("News_Articles", merged_news_articles, key_columns=["symbol", "url"])
            self.db.upsert_table("News_Scores", merged_news_scores, key_columns=["symbol"])
            self.db.upsert_table("App_Input_View", app_view, key_columns=["symbol"], delete_missing=True)
            self.db.write_table("Provider_Log", provider_log_df)
            self.db.write_table("Refresh_Control", refresh_control)
            self.db.write_table("Config", config_df)
        except Exception as exc:
    ...
    def _merge_with_cached_history(self, universe: pd.DataFrame, fresh: pd.DataFrame) -> pd.DataFrame:
        ...
        return self._merge_history_frames(cached, current)

    def _merge_history_frames(self, cached: pd.DataFrame, fresh: pd.DataFrame) -> pd.DataFrame:
        frames = [df for df in [cached, fresh] if df is not None and not df.empty]
        if not frames:
        ...
        merged = merged.dropna(subset=["date", "close"]).sort_values(["symbol", "date"])
        merged = merged.drop_duplicates(subset=["symbol", "date"], keep="last")
        merged = merged.groupby("symbol", group_keys=False).tail(HISTORY_TRADING_DAYS)
        cols = ["symbol", "date", "open", "high", "low", "close", "prev_close", "volume", "value", "source"]
        for col in cols:
            if col not in merged.columns:
                merged[col] = np.nan if col not in {"symbol", "source"} else ""
        return merged[cols].reset_index(drop=True)

    def _extract_incremental_history_rows(self, history_df: pd.DataFrame) -> pd.DataFrame:
        ...
        current = current.dropna(subset=["symbol", "date", "close"]).drop_duplicates(subset=["symbol", "date"], keep="last")
        cached = self._cached_history.copy()
        if cached.empty:
        ...
        cached_keys = cached[["symbol", "date"]].dropna().drop_duplicates().assign(_cached=1)
        merged = current.merge(cached_keys, on=["symbol", "date"], how="left")
        return merged[merged["_cached"].isna()].drop(columns=["_cached"]).reset_index(drop=True)
    ...
    def _merge_with_cached_news_articles(self, fresh: pd.DataFrame) -> pd.DataFrame:
        current = fresh.copy() if fresh is not None else pd.DataFrame()
        cached = self._cached_news_articles.copy()
        frames = [df for df in [current, cached] if df is not None and not df.empty]
        if not frames:
            return pd.DataFrame()
        current = pd.concat(frames, ignore_index=True)
        if "url" in current.columns:
            current["url"] = current["url"].astype(str)
            dedupe_cols = [c for c in ["symbol", "url"] if c in current.columns]
            if dedupe_cols:
                current = current.drop_duplicates(subset=dedupe_cols, keep="first")
        return current.reset_index(drop=True)

    def _merge_with_cached_news_scores(self, fresh: pd.DataFrame) -> pd.DataFrame:
        current = fresh.copy() if fresh is not None else pd.DataFrame()
        cached = self._cached_news_scores.copy()
        frames = [df for df in [current, cached] if df is not None and not df.empty]
        if not frames:
            return pd.DataFrame()
        current = pd.concat(frames, ignore_index=True)
        if "symbol" in current.columns:
            current["symbol"] = current["symbol"].astype(str).str.upper().str.strip()
            current = current.drop_duplicates(subset=["symbol"], keep="first")
        return current.reset_index(drop=True)
    ...
    def _get_best_history(self, row: dict, from_dt: datetime, to_dt: datetime) -> Tuple[pd.DataFrame, str, List[str], float]:
        ...
        latest_cached_date = pd.NaT
        if not cached.empty:
            cached["date"] = pd.to_datetime(cached["date"], errors="coerce").dt.normalize()
            cached = cached.dropna(subset=["date"])
            latest_cached_date = cached["date"].max() if not cached.empty else pd.NaT
            if pd.notna(latest_cached_date):
                cache_age_days = max((pd.Timestamp(to_dt).normalize() - latest_cached_date).days, 0)
                if cache_age_days <= 1 and len(cached) >= min(HISTORY_TRADING_DAYS, 180):
                    return cached, "CACHE_FRESH", errors, (time.perf_counter() - started) * 1000

        effective_from_dt = from_dt
        partial_refresh = False
        if pd.notna(latest_cached_date):
            partial_refresh = True
            effective_from_dt = max(from_dt, latest_cached_date.to_pydatetime() - timedelta(days=10))
        ...
        if nse_symbol:
            try:
                ...
                if not hist_df.empty:
                    merged = self._merge_history_frames(cached, hist_df) if partial_refresh else hist_df
                    return merged, ("NSE_INCREMENTAL" if partial_refresh else "NSE"), errors, (time.perf_counter() - started) * 1000
        ...
        for ticker, source in [(row.get("yahoo_ticker_ns"), "YAHOO_NS"), (row.get("yahoo_ticker_bo"), "YAHOO_BO")]:
            ...
            try:
                ...
                if not hist_df.empty:
                    merged = self._merge_history_frames(cached, hist_df) if partial_refresh else hist_df
                    source_name = f"{source}_INCREMENTAL" if partial_refresh else source
                    return merged, source_name, errors, (time.perf_counter() - started) * 1000
