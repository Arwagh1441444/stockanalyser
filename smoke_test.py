...
def main() -> None:
    ...
    assert "recommendation_confidence" in app_view.columns
    assert "recommendation_reason" in app_view.columns
    row = app_view.iloc[0]
    assert float(row["history_days"]) >= 0
    assert row["recommendation"] in {"BUY", "HOLD", "SELL", "NO DATA"}
