...
def refresh_market_database(db_path: str, sample_limit: int, news_limit: int, refresh_universe: str) -> None:
...
def main() -> None:
    ...
    with st.sidebar:
        ...
        if st.button("API Update / Refresh Database", width='stretch'):
            with st.spinner("Refreshing market prices, history, and news scores..."):
                refresh_market_database(str(db_path), int(sample_limit), int(news_limit), refresh_universe)
                load_bundle.clear()
    ...
    with tabs[1]:
        ...
        if stock_news.empty:
            ...
            st.dataframe(display_news.head(12), width='stretch', hide_index=True)
    ...
    with tabs[2]:
        ...
        with n1:
            ...
            st.dataframe(bullish[["symbol", "company_name", "news_score", "news_article_count_7d", "headline_risk_flag", "top_positive_headline"]], width='stretch', hide_index=True)
        with n2:
            st.markdown("#### Weaker / riskier news flow")
            st.dataframe(bearish[["symbol", "company_name", "news_score", "news_article_count_7d", "headline_risk_flag", "top_negative_headline"]], width='stretch', hide_index=True)
    ...
    with tabs[3]:
        ...
        st.dataframe(provider_summary(logs), width='stretch', hide_index=True)
        if cfg:
            st.markdown("### Engine config")
            st.dataframe(pd.DataFrame([{"key": k, "value": v} for k, v in cfg.items()]), width='stretch', hide_index=True)
