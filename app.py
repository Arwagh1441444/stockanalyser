...
def latest_portfolio_news(news_articles: pd.DataFrame, symbols: List[str]) -> pd.DataFrame:
    ...
    return out.sort_values(["published_at", "article_sentiment_raw"], ascending=[False, True])
...
def main() -> None:
    ...
    pre_db = MarketDatabase(str(db_path))
    existing_portfolio = normalize_portfolio(pre_db.read_table(PORTFOLIO_TABLE)) if db_path.exists() else normalize_portfolio(pd.DataFrame())
    ...
    with tabs[0]:
        ...
        with add_col:
            with st.form("portfolio_add_form", clear_on_submit=True):
                ...
                selected_add_symbol = st.selectbox(
                    ...
                    format_func=lambda s: f"{s} · {app_view.loc[app_view['symbol'] == s, 'company_name'].iloc[0] if not app_view[app_view['symbol'] == s].empty else s}",
                )
                p1, p2 = st.columns(2)
                ...
                if submitted:
                    company_name = app_view.loc[app_view["symbol"] == selected_add_symbol, "company_name"].iloc[0]
        ...
        if portfolio_view.empty:
            ...
            portfolio_news = latest_portfolio_news(news_articles, portfolio_symbols(portfolio_df))
