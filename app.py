...
def ensure_static_dashboard_table(db_path: Path) -> None:
    db = MarketDatabase(str(db_path))
    db.initialize()
    current = db.read_table(STATIC_DASHBOARD_TABLE)
    if current is None or current.empty:
        db.write_table(STATIC_DASHBOARD_TABLE, pd.DataFrame(STATIC_NIFTY_DASHBOARD_DATA))


def load_static_dashboard_data(db_path: Path) -> pd.DataFrame:
    ensure_static_dashboard_table(db_path)
    db = MarketDatabase(str(db_path))
    df = db.read_table(STATIC_DASHBOARD_TABLE)
    if df is None or df.empty:
        return pd.DataFrame(STATIC_NIFTY_DASHBOARD_DATA)
    for col in ["Current", "High", "Low", "PHigh", "PLow"]:
...
def render_static_nifty_dashboard(static_dashboard_df: pd.DataFrame) -> None:
    ...
    <body>
      ...
      <script>
        const data = {json.dumps(static_dashboard_df.to_dict(orient="records"), ensure_ascii=False)};
...
def main() -> None:
    ...
    ensure_static_dashboard_table(db_path)
    static_dashboard_df = load_static_dashboard_data(db_path)
    ...
    if app_view.empty:
        st.warning("Market tables are empty right now. Refresh database for live tabs. Static Dashboard is available below.")
        tabs = st.tabs(["Static Dashboard"])
        with tabs[0]:
            render_static_nifty_dashboard(static_dashboard_df)
        return
